...
const universities = [
  {
    uni: 'Bells University of Technology',
  },
  {
...
const Footer = () => {
  const [currentUni, setCurrentUni] = useState<string>('MisplaceMe');

  useEffect(() => {
    const fetchUserData = async () => {
      const userUniversity = ''
      const university = universities.find((uni) => uni.uni === userUniversity);
      setCurrentUni(university?.uni || 'MisplaceMe');
    };
  ...
  return (
    <footer className="flex flex-col items-center border-t-[2px] py-2 md:py-4">
      <div className="flex justify-center gap-4 lg:gap-10 text-base md:text-lg font-normal">
        <p className="cursor-pointer active:text-blue-700">Terms & Conditions</p>
        <p className="cursor-pointer active:text-blue-700">Privacy policy</p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <p className="font-medium text-base md:text-lg">{currentUni}</p>
      </div>
    </footer>
  )
}
